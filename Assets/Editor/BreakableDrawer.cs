...
public class BreakableDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        SerializedProperty breakSound = property.FindPropertyRelative("breakSound");
        SerializedProperty breakAudioSource = property.FindPropertyRelative("breakAudioSource");
        SerializedProperty breakVolume = property.FindPropertyRelative("breakVolume");
        ...
        Rect breakSoundRect = new(position.x, position.y + totalHeight, position.width, EditorGUI.GetPropertyHeight(breakSound));
        totalHeight += EditorGUI.GetPropertyHeight(breakSound) + 2;

        Rect breakAudioSourceRect = new(position.x, position.y + totalHeight, position.width, EditorGUI.GetPropertyHeight(breakAudioSource));
        totalHeight += EditorGUI.GetPropertyHeight(breakAudioSource) + 2;

        Rect breakVolumeRect = new(position.x, position.y + totalHeight, position.width, EditorGUI.GetPropertyHeight(breakVolume));
        totalHeight += EditorGUI.GetPropertyHeight(breakVolume) + 2;
        ...
        EditorGUI.PropertyField(breakSoundRect, breakSound);
        EditorGUI.PropertyField(breakAudioSourceRect, breakAudioSource);
        EditorGUI.PropertyField(breakVolumeRect, breakVolume);
